        ...
        <div className="flex flex-col gap-6">
          ...
          >
            ...
            <div>
              ...
              <ul className="list-disc list-inside space-y-2" style={{ marginLeft: "1rem" }}>
                <li><strong>Scaling from local to global:</strong> how we went from per-school databases of ~10k profiles to a unified architecture indexing 800M+ people and 30M companies worldwide.</li>
                <li><strong>From FAISS to OpenSearch:</strong> why dense-only retrieval broke down at scale and how we evolved toward a multi-layer retrieval stack (MySQL + OpenSearch).</li>
                <li><strong>Cost vs. latency trade-offs:</strong> lessons from using BigQuery (great indexing, terrible cost per TB scanned) and the iterative optimizations that made real-time search feasible.</li>
                <li><strong>Parallel ingestion at extreme scale:</strong> engineering a two-stage sequential pipeline that ingested 1.6 billion records in &lt; 24 hours with atomic checkpointing and zero data loss.</li>
                <li><strong>Agentic orchestration:</strong> building a prompt-to-SQL agent swarm capable of evaluating candidates and scraping additional signals with asyncio + Firecrawl.</li>
                <li><strong>Embeddings at scale:</strong> self-hosting Qwen Embedding 0.6 on a distributed Runpod + NGINX cluster to generate hundreds of millions of embeddings at 12-15k EPS per GPU.</li>
                <li><strong>Agentic chunking experiments:</strong> using LLMs to summarize each profile into multiple semantic facets.</li>
                <li><strong>Hybrid retrieval (sparse + dense):</strong> evaluating Milvus BM25 + vector hybrid search, and why query-term explosion and large-scale union merges became prohibitively expensive at 800M profiles.</li>
                <li><strong>Final architecture:</strong> a 8 TB OpenSearch index backed by a 25 TB Aurora MySQL store, achieving ~4 s end-to-end query latency for global-scale people search.</li>
              </ul>
            </div>

            <p>
              You can play around with a deployed instance of this search engine on{" "}
              <a
            ...
            <div>
              <h4 className="font-bold" style={{ color: subheadingColor, marginBottom: "0.5rem" }}>
                People Ingestion Architecture:
              </h4>
              <ul className="list-disc list-inside space-y-2" style={{ marginLeft: "1rem" }}>
                <li><strong>375 parallel worker processes</strong> - We pushed Python's multiprocessing to its limits, spawning hundreds of workers that could each handle their own parquet file independently</li>
                <li><strong>Subprocess isolation</strong> - Each worker ran as a separate Python process to avoid GIL contention and memory leaks</li>
                <li><strong>Atomic checkpointing</strong> - Workers appended completed files to a checkpoint file atomically, allowing us to resume from failures without data loss</li>
                <li><strong>Retry mechanism</strong> - Failed files got up to 3 retries before being logged to an error file for manual inspection</li>
              </ul>
            </div>
            ...
            <div>
              ...
              <ul className="list-disc list-inside space-y-2" style={{ marginLeft: "1rem" }}>
                <li><strong>64 parallel workers</strong> with larger batch sizes (50,000 records)</li>
                <li><strong>Vectorized DataFrame transformations</strong> - Instead of row-by-row processing, we used pandas vectorized operations to transform entire columns at once</li>
                <li><strong>Bulk INSERT with ON DUPLICATE KEY UPDATE</strong> - MySQL's bulk insert with upsert semantics meant we could handle duplicates efficiently</li>
                <li><strong>Temporary file optimization</strong> - We used /mnt/dumps/ on high-speed NVMe drives for temporary parquet storage, falling back to system temp only when needed</li>
              </ul>
            </div>
            ...
            <div>
              ...
              <p style={{ marginBottom: "0.5rem" }}><strong>Extraction Phase (MySQL → Parquet):</strong></p>
              <ul className="list-disc list-inside space-y-2" style={{ marginLeft: "1rem", marginBottom: "1rem" }}>
                <li><strong>128 parallel extraction workers</strong> each assigned a specific ID range using MySQL partition pruning</li>
                <li><strong>ID-based sharding</strong> - Each worker scanned a range like id &gt; 100M AND id &lt;= 107M, ensuring no overlap</li>
                <li><strong>LEFT JOIN optimization</strong> - We pre-joined people with companies in MySQL, avoiding the need for lookups during indexing</li>
                <li><strong>50,000 record batches</strong> - Large enough for efficiency, small enough to fit in memory</li>
              </ul>

              <p style={{ marginBottom: "0.5rem" }}><strong>Processing Phase (Parquet → OpenSearch):</strong></p>
              <ul className="list-disc list-inside space-y-2" style={{ marginLeft: "1rem" }}>
                <li><strong>File-based queueing system</strong> - We used atomic file operations (rename) to coordinate between extraction and processing stages</li>
                <li><strong>24 concurrent processors</strong> - Each handling document transformation and bulk indexing</li>
                <li><strong>10,000 document bulk operations</strong> - OpenSearch's sweet spot for bulk indexing performance</li>
                <li><strong>Continuous processing</strong> - Processors started working as soon as the first parquet files appeared, no waiting for extraction to complete</li>
              </ul>
            </div>
            ...
            <div>
              ...
              <ul className="list-disc list-inside space-y-2" style={{ marginLeft: "1rem" }}>
                <li>Garbage collection every 10 batches to prevent memory bloat</li>
                <li>Worker process recycling to avoid long-term memory leaks</li>
                <li>Streaming processing - never loading the full dataset into memory</li>
              </ul>
            </div>
            ...
            <div>
              ...
              <ul className="list-disc list-inside space-y-2" style={{ marginLeft: "1rem" }}>
                <li>NVMe SSDs for temporary storage with automatic fallback to system temp</li>
                <li>Snappy compression for parquet files - fast compression with reasonable ratios</li>
            ...
            <div>
              ...
              <ul className="list-disc list-inside space-y-2" style={{ marginLeft: "1rem" }}>
                <li>Checkpoint files tracked both completed batches and continuous progress</li>
                <li>Failed batches moved to a "failed" queue for retry or manual inspection</li>
                <li>Graceful degradation - if one worker failed, others continued</li>
              </ul>
            </div>
            ...
            <div>
              ...
              <ul className="list-disc list-inside space-y-2" style={{ marginLeft: "1rem" }}>
                ...
                <li>Efficient handling of nested structures (experiences, education, etc.)</li>
              </ul>
            </div>
            ...
            <ul className="list-disc list-inside space-y-2" style={{ marginLeft: "1rem" }}>
              <li><strong>22 hours</strong> total ingestion time for 800M profiles</li>
              <li><strong>~10,100 profiles/second</strong> sustained throughput</li>
              <li><strong>Zero data loss</strong> through atomic checkpointing</li>
              <li><strong>4-second</strong> end-to-end query latency on the final index</li>
            </ul>
            ...
            <p>
              Afterwards, we tried a hybrid approach with Milvus by combining sparse and dense embeddings. However, with Milvus, the issue that we ran into is the fact that as the number of keywords increased, query latency grew significantly. Moreover, the fusion stage between the sparse and dense retrieval results became extremely resource-intensive, since for a corpus of over 800 million profiles, the hybrid search requires retrieving and merging a large number of candidates from both the BM25 (sparse) and vector (dense) indexes. This meant that even with parallel execution, the union and re-ranking process ballooned in cost, making it difficult to sustain real-time latency at scale.
            </p>
